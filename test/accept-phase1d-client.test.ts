...
function passingResult() {
  return {
    ...
    knowledgePointEdited: true,
    editProviderCalls: 0,
    deletedFixtureGone: true,
    existingRunRetained: true,
    deleteProviderCalls: 0,
    filePreview: {
      filename: 'sample.txt',
...
describe('Phase 1D browser acceptance result', () => {
  ...
  test.each([
    ...
    ['knowledge point edit not persisted', (value: ReturnType<typeof passingResult>) => { value.knowledgePointEdited = false }, 'CLIENT_CONTENT_MANAGEMENT_INVALID'],
    ['editing calls provider', (value: ReturnType<typeof passingResult>) => { value.editProviderCalls = 1 }, 'CLIENT_CONTENT_MANAGEMENT_INVALID'],
    ['deleted fixture remains', (value: ReturnType<typeof passingResult>) => { value.deletedFixtureGone = false }, 'CLIENT_CONTENT_MANAGEMENT_INVALID'],
    ['existing run was deleted', (value: ReturnType<typeof passingResult>) => { value.existingRunRetained = false }, 'CLIENT_CONTENT_MANAGEMENT_INVALID'],
    ['deleting calls provider', (value: ReturnType<typeof passingResult>) => { value.deleteProviderCalls = 1 }, 'CLIENT_CONTENT_MANAGEMENT_INVALID'],
