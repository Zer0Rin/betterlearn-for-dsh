...
- [ ] 在generation-adapter/model-selection-propagation测试中覆盖L1、L2、L3含边界计数；只统计owned child真实chunk，释放监听，节流与无响应时间不伪造。
- [ ] 在coordinator/routes/client-api/poll-run测试中覆盖内存快照、SSE首连及推送、断开轮询恢复、迟到进度不得覆盖新任务或终态，不新增模型调用。
- [ ] 在RunProgress和workspace测试中覆盖规划中/第N批/最后响应/无数据/终态清除；实现最小界面，保留现有失败和恢复行为。
- [ ] 构建与定向测试；隔离真实rc.8假模型以可观察延迟验证请求→进度→审核，浏览器检查展示与刷新恢复，故意断SSE验证兜底。禁止真实模型调用。
- [ ] 打包0.0.3；确认本地没有活动提取后备份升级重启，保留已有53条候选及审核决定；记录验收、提交代码，未跟踪vendor目录不触碰。
