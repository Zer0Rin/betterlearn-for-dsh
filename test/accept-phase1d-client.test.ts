...
function passingResult() {
  return {
    ...
    fakeLedger: {
      nonce: 'fake-nonce',
      records: [{ provider: 'deepseek-official', model: 'deepseek-v4-flash', toolNames: ['structured_output'], result: 'structured' }],
    },
    screenshots: {
...
describe('Phase 1D browser acceptance result', () => {
  ...
  test.each([
    ...
    ['missing review request', (value: ReturnType<typeof passingResult>) => { value.productRequests.splice(4, 1) }, 'CLIENT_PRODUCT_REQUEST_FAILED'],
    ['wrong review method', (value: ReturnType<typeof passingResult>) => { value.productRequests[2].method = 'GET' }, 'CLIENT_PRODUCT_REQUEST_FAILED'],
    ['failed product request', (value: ReturnType<typeof passingResult>) => { value.productRequests[2].status = 409 }, 'CLIENT_PRODUCT_REQUEST_FAILED'],
