...
try {
  ...
  await page.getByTestId('learning-book-preview-kp_chunking').click()
  ...
  await page.getByTestId('nobei-create-learning-book').click()
  assert.equal(await panel.getAttribute('data-area'), 'library')
  assert.equal(await page.getByTestId('learning-lesson').count(), 0)
  await page.getByTestId('learning-book-preview-kp_chunking').click()
  assert.equal(await panel.getAttribute('data-mode'), 'learning')
  ...
  console.log('learning preview verified: gateways, bookshelf, course return, remediation, sidebars, narrow layout')
} finally {
  await browser.close()
}
